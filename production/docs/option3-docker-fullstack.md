...
Layer `docker-compose.fullstack.viz.yml` on top of the fullstack file to add the [3DCityDB Web Map Client](https://github.com/3dcitydb/3dcitydb-web-map) to the Chat tab:

```bash
docker compose -f production/docker-compose.fullstack.yml -f production/docker-compose.fullstack.viz.yml up -d
```

It sets `ENABLE_VIZ=true` and adds a `tiles_data` volume for the 3D tiles generated by [citydb-3dtiler](https://github.com/tum-gis/citydb-3dtiler); nothing else changes.

- Tiles are generated automatically after an import (*Generate 3D tiles after import*), or on demand with **🔄 Refresh 3D tiles** above the viewer.
- When an answer lists `objectid`s, the viewer highlights them (everything else is ghosted; **Isolate** hides it) and flies to them. Features without geometry of their own, e.g. a `Room`, are expanded to the surfaces they contain.
- The web map client is baked into the agent image, so rebuild after updating: `docker compose -f production/docker-compose.fullstack.yml build citydb-agent`.
- Pre-pull the tiler image (~1.7 GB): `docker compose -f production/docker-compose.fullstack.yml -f production/docker-compose.fullstack.viz.yml --profile tiler pull`.
